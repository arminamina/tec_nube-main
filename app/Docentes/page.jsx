...
import ModalEliminarDocentes from "./ModalEliminarDocentes";
...
export default function Docentes() {
  ...
  useEffect(() => {
    const obtenListadoArticulosDocentes = async () => {
      try {
        // Hacer el request GET
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/Docentes/ListadoDocentes`,
        );
  ...
  return (
    <div className="container-fluid">
      <h1>Listado de Articulos Publicados en Docentes</h1>
      <div style={{ textAlign: "right" }} className="mb-3">
        <a href="/Docentes/Nueva" className="btn btn-success">
          Nueva Publicacion
        </a>
      </div>
            ...
            {general.map((item) => {
              return (
                <tr key={item.idArticulo} className="odd">
                  ...
                  >
                    <a
                      href={`/Docentes/Editar?id=${item.idArticulo}`}
