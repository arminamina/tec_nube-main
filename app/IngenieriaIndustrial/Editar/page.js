    ...
    const fetchArticulo = async () => {
      try {
        const res = await fetch(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/IngIndustrial/GetEditarArticuloIndustrial/${id}`,
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_BASE_URL}/IngIndustrial/ActualizarArticuloIndustrial/${id}`,
        {
          method: "PUT",
          headers: {
