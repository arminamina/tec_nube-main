...
import ModalEliminarIndustrial from "./ModalEliminarIndustrial";
...
export default function Industrial() {
  ...
  useEffect(() => {
    const obtenListadoArticulosIndustrial = async () => {
      try {
        // Hacer el request GET
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/IngIndustrial/ListadoIndustrial`,
        );
        // Actualizar el estado de datos
        setIndustrial(response.data);
        setCargando(false);
      } catch (error) {
        // En caso de error
        setError(error);
    ...
    // llamar a la funcion
    obtenListadoArticulosIndustrial();
  }, []);
  ...
  return (
    <div className="container-fluid">
      <h1>Listado de Articulos Publicados en Ing. Industrial</h1>
      <div style={{ textAlign: "right" }} className="mb-3">
        <a href="/IngenieriaIndustrial/Nueva" className="btn btn-success">
          Nueva Publicacion
        </a>
      </div>
            ...
            {industrial.map((item) => {
              return (
                <tr key={item.idArticulo} className="odd">
                  ...
                  >
                    <a
                      href={`/IngenieriaIndustrial/Editar?id=${item.idArticulo}`}
