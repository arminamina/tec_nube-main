...
import ModalEliminarIM from "./ModalEliminarIM";
...
export default function IM() {
  ...
  useEffect(() => {
    const obtenListadoArticulosIM = async () => {
      try {
        // Hacer el request GET
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/IM/ListadoIM`,
        );
  ...
  return (
    <div className="container-fluid">
      <h1>Listado de Articulos Publicados en Ingenieria en Minas</h1>
      <div style={{ textAlign: "right" }} className="mb-3">
        <a href="/IM/Nueva" className="btn btn-success">
          Nueva Publicacion
        </a>
      </div>
            ...
            {im.map((item) => {
              return (
                <tr key={item.idArticulo} className="odd">
                  ...
                  >
                    <a
                      href={`/IM/Editar?id=${item.idArticulo}`}
