...
import ModalEliminarIGE from "./ModalEliminarIGE";

export default function IngeGestion() {
  ...
  useEffect(() => {
    const obtenListadoArticulosIGE = async () => {
      try {
        // Hacer el request GET
        const response = await axios.get(
          `${process.env.NEXT_PUBLIC_API_BASE_URL}/IGE/ListadoGeneral`,
        );
  ...
  return (
    <div className="container-fluid">
      <h1>Listado de Articulos Publicados en IGE</h1>
      <div style={{ textAlign: "right" }} className="mb-3">
        <a href="/IGE/NuevaIGE" className="btn btn-success">
          Nueva Publicacion
        </a>
      </div>
            ...
            {general.map((item) => {
              return (
                <tr key={item.idArticulo} className="odd">
                  ...
                  >
                    <a
                      href={`/IGE/EditarIGE?id=${item.idArticulo}`}
